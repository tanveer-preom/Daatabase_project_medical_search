...
class PageController extends Controller {
    ...
    {
        $test_id = input::get('t_id');
        $diagnostic_center_id= input::get('d_id');
        return 'Test id = '.$test_id."\nDiagnostic center id=".$diagnostic_center_id;
        return view('details');
    }
    ...
    {   
        ...
        //dd($result);
            ...
            $dummy= DB::table('test')
                    ...
                    ->select('test.id as test_id','company.name as name','test.name as TestName','diagnostic_center_and_test.diagnostic_center_id as diagId','diagnostic_center.name as DiagName',
                        'diagnostic_center_and_test.price as DiagPrice',
                        'diagnostic_center_and_test.additional_info as additional_info')
                    -> where('test_id',$test);
       ...
       */
        ...
        */
        ...
        return view('search_result')->with('results',$test)->with('tests',$tests)->with('test_category',$test_category)
            ->with('diag_area',$diag_area)->with('diag_district',$diag_district)->with('userId',$ID);
    }
